...
export class ToursService {
  ...
  /**
   * P0-6.7 TEMP: Diagnose der FV-Eligibility-Pipeline.
   * Liefert Counts der 5 SQL-Quick-Checks + Service-Cache-State.
   * Cleanup-Backlog P0-6.8 (Endpoint löschen wenn Bug gefixt).
   */
  async eligibleShipmentsFvDebug() {
    const [
      shipmentsOpenInSet,
      byStatusTransport,
      pickupCompletedStops,
      pickupCompletedShipmentsSample,
      activeRelations,
    ] = await Promise.all([
      ...
      this.prisma.shipments.groupBy({
        by: ['status', 'transport_type'],
        where: { tour_id: null, deleted_at: null },
        _count: { _all: true },
      }),
      this.prisma.nv_tour_stops.count({
        where: {
          stop_type: 'PICKUP',
          nv_tour: { status: 'COMPLETED' },
        },
      }),
      this.prisma.nv_tour_stops.findMany({
        where: {
          stop_type: 'PICKUP',
          nv_tour: { status: 'COMPLETED' },
        },
      ...
      this.prisma.relations.count({ where: { is_active: true } }),
    ]);

    const fvRelations = await this.getOwnFvRelationsSet();
    const nvPlz = await getNvPlzSet(this.prisma as any);
    const eligibleNow = await this.eligibleShipmentsFv({});

    return {
      counts: {
        shipments_in_status_set_tour_null: shipmentsOpenInSet,
        pickup_completed_stops: pickupCompletedStops,
        active_relations: activeRelations,
        eligible_now: eligibleNow.length,
      },
      by_status_transport: byStatusTransport,
      pickup_completed_sample: pickupCompletedShipmentsSample.map(
        (s) => s.shipment,
      ),
      service_state: {
        fvRelations_size: fvRelations.size,
        nv_plz_exact_count: nvPlz.exact.size,
        nv_plz_prefixes_count: nvPlz.prefixes.length,
        nv_plz_prefixes: nvPlz.prefixes,
      },
    };
  }
