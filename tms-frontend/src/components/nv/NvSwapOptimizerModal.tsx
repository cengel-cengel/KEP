...
export default function NvSwapOptimizerModal({
  ...
  // Phase 2: Liste anderer NV-Touren fuer Manual-Picker. Filtert
  // Source-Tour raus + auf 'PLANNING'-Status. Augmented-loop liefert
  // tour.overload (O-3) → capacityHint per Option.
  type NvTourLite = {
    id: string;
    status?: string | null;
    datum?: string | null;
    nv_stamm_tour?: { code?: string | null } | null;
    subunternehmer?: { name?: string | null } | null;
    overload?: { vol?: number; weight?: number } | null;
  };
  const toursQ = useQuery<NvTourLite[]>({
    queryKey: ['nv-touren'],
    queryFn: async () => (await api.get<NvTourLite[]>('/nv-touren')).data,
    staleTime: 30_000,
  });
  const manualTourOptions: TourOption[] = useMemo(() => {
    ...
    return all
      .filter(
        (t) => t.id !== sourceTourId && (t.status ?? '') === 'PLANNING',
      )
      .map((t) => {
        const labelBase =
          t.nv_stamm_tour?.code ?? t.subunternehmer?.name ?? t.id.slice(0, 8);
        const datumShort = t.datum ? t.datum.slice(0, 10) : null;
