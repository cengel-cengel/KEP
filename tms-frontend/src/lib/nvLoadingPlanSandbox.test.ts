...
describe('sandboxReducer', () => {
  ...
  describe('makeKey / parseKey (H5b)', () => {
    it('round-trip dbItemId + paletteIndex', () => {
      const k = makeKey('i-1', 2);
      expect(k).toBe('i-1|2');
      expect(parseKey(k)).toEqual({ dbItemId: 'i-1', paletteIndex: 2 });
    });
    it('parseKey no separator → paletteIndex 0 fallback', () => {
      expect(parseKey('i-1')).toEqual({ dbItemId: 'i-1', paletteIndex: 0 });
    });
  });

  describe('setPosition', () => {
    it('fuegt Override hinzu (key = dbItemId|paletteIndex)', () => {
      ...
      expect(s.positionOverrides.get(makeKey('i-1', 0))).toEqual(pos);
    });
    ...
    it('ueberschreibt existierenden Override (Drag erneut)', () => {
      ...
      expect(s2.positionOverrides.get(makeKey('i-1', 0))).toEqual({
        posXCm: 50,
        posYCm: 60,
        posZCm: 70,
      });
    ...
    it('rotationDeg wird durchgereicht', () => {
      ...
      expect(s.positionOverrides.get(makeKey('i-1', 0))?.rotationDeg).toBe(90);
    });

    it('H5b: gleiche dbItemId, verschiedene paletteIndex → 2 Entries', () => {
      ...
      s = sandboxReducer(s, {
        ...
        dbItemId: 'i-1',
        paletteIndex: 1,
        pos: { posXCm: 100, posYCm: 200, posZCm: 0 },
      });
      expect(s.positionOverrides.size).toBe(2);
      expect(s.positionOverrides.get(makeKey('i-1', 0))?.posXCm).toBe(10);
      expect(s.positionOverrides.get(makeKey('i-1', 1))?.posXCm).toBe(100);
    });
  });

  describe('clearPosition', () => {
    it('entfernt Override (paletteIndex-spezifisch)', () => {
      const s1 = sandboxReducer(initialSandboxState, {
        type: 'setPosition',
        dbItemId: 'i-1',
        paletteIndex: 0,
    ...
    it('H5b: clearPosition trifft nur den spezifischen paletteIndex', () => {
      let s = sandboxReducer(initialSandboxState, {
        type: 'setPosition',
        dbItemId: 'i-1',
        paletteIndex: 0,
      ...
      expect(s.positionOverrides.size).toBe(1);
      expect(s.positionOverrides.has(makeKey('i-1', 1))).toBe(true);
    });
  });
  ...
  describe('Akkumulation (Übernehmen-Vollständigkeit)', () => {
    it('mehrere setPosition + eject akkumulieren korrekt', () => {
      ...
      expect(s.positionOverrides.get(makeKey('i-A', 0))?.posXCm).toBe(10);
      expect(s.positionOverrides.get(makeKey('i-B', 0))?.posXCm).toBe(30);
    });
